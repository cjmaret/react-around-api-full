...
function Card(props) {
  ...
  const cardDeleteButtonClassName = `image-card__trash ${
    isOwn ? "image-card__trash_visible" : "image-card__trash_hidden"
  }`;
  const isLiked = props.card.likes?.some((i) => i === currentUser?._id);
  const cardLikeButtonClassName = `image-card__heart ${
    isLiked ? "image-card__heart_liked" : ""
  }`;
